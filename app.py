...
def load_data():
    ...
    if opt.dataset == 'Pinwheel':
        from toy_datasets import load_pinwheel_data
        print("使用toy_datasets.py生成Pinwheel数据集...")
        train_data, val_data, test_data = load_pinwheel_data()
        train_data = [[list(i) for i in seq] for seq in train_data]
        val_data = [[list(i) for i in seq] for seq in val_data]
        test_data = [[list(i) for i in seq] for seq in test_data]
    elif opt.dataset == 'HawkesGMM':
        from toy_datasets import load_gmm_data
        print("使用toy_datasets.py生成HawkesGMM数据集...")
        train_data, val_data, test_data = load_gmm_data()
        train_data = [[list(i) for i in seq] for seq in train_data]
        val_data = [[list(i) for i in seq] for seq in val_data]
        test_data = [[list(i) for i in seq] for seq in test_data]
    else:
...
def data_loader(writer):
    ...
    train_data = [u for u in train_data if len(u) > 0]
    val_data = [u for u in val_data if len(u) > 0]
    test_data = [u for u in test_data if len(u) > 0]
...
def Batch2toModel(batch, transformer):
    ...
    enc_out, mask, background_feat = transformer(event_loc, event_time_origin)

    enc_out_non_mask  = []
    event_time_non_mask = []
    event_loc_non_mask = []
    background_feat_non_mask = []
    for index in range(mask.shape[0]):
        ...
        if length>1:
            ...
            background_feat_non_mask += [i.unsqueeze(dim=0) for i in background_feat[index][:length-1]]
    ...
    background_feat_non_mask = torch.cat(background_feat_non_mask, dim=0)  # 新增
    ...
    background_feat_non_mask = background_feat_non_mask.reshape(event_time_non_mask.shape[0], 1, -1)

    return event_time_non_mask, event_loc_non_mask, enc_out_non_mask,background_feat_non_mask
...
if __name__ == "__main__":
    ...
    logdir = "./logs/{}_timesteps_{}_{}".format(opt.dataset, opt.timesteps, run_id)
    model_path = './ModelSave/dataset_{}_timesteps_{}/{}'.format(opt.dataset, opt.timesteps, run_id)
    ...
    for itr in range(opt.total_epochs):
        ...
        if itr % 10==0:
            ...
            with torch.no_grad():
                ...
                for batch in valloader:
                    event_time_non_mask, event_loc_non_mask, enc_out_non_mask, background_feat = Batch2toModel(batch, Model.transformer)
                    ...
                    sampled_seq_temporal_all = torch.stack(sampled_seq_temporal_all, dim=0)  # [100, batch, 1]
                    sampled_seq_spatial_all = torch.stack(sampled_seq_spatial_all, dim=0)    # [100, batch, 1, dim]
                    ...
                    # # print("sampled_seq_spatial_all type:", type(sampled_seq_spatial_all))
                    # # print("real:", real[:5] if hasattr(real, "__getitem__") else real)
                    # # print("sampled_seq_spatial_all:", sampled_seq_spatial_all[:5] if hasattr(sampled_seq_spatial_all, "__getitem__") else sampled_seq_spatial_all)
                    # assert real.shape==gen.shape
                    ...
                    assert real.shape == gen.shape  # ok
                    real_expanded = real.unsqueeze(0).unsqueeze(1)   # [1, batch, 1, dim]
                    ...
                    del sampled_seq, sampled_seq_temporal_all, sampled_seq_spatial_all, real, gen, real_expanded
                    del event_time_non_mask, event_loc_non_mask, enc_out_non_mask, background_feat
                    torch.cuda.empty_cache()
                ...
                for batch in testloader:
                    ...
                    loss_test_all += loss.item() * batch_size
                    total_num += batch_size
                    del sampled_seq, loss, event_time_non_mask, event_loc_non_mask, enc_out_non_mask, background_feat
                    torch.cuda.empty_cache()
        ...
        for batch in trainloader:
            ...
            del loss, event_time_non_mask, event_loc_non_mask, enc_out_non_mask, background_feat
            if step % 10 == 0:  
                torch.cuda.empty_cache()
