...
try:
    ...
    # SpatioTemporalDataset不是必需的，只有在使用PinwheelHawkes或GMMHawkes类时才需要
    SpatioTemporalDataset = None
...
if SpatioTemporalDataset is not None:
...
def generate(mhp, data_fn, ndim, num_classes):
    ...
    event_times = mhp.data[:, 0]
    classes = mhp.data[:, 1].astype(int)
    n = len(event_times)
...
def load_pinwheel_data():
    """
    生成Pinwheel数据集
    返回: (train_data, val_data, test_data)
    每个数据是一个列表，每个元素是一个numpy数组，形状为(n, 3)，其中第一列是时间，后两列是x, y坐标
    """
    num_classes = 10
    m = np.array([0.05] * num_classes)
    a = np.diag([0.6] * (num_classes - 1), k=-1) + np.diag([0.6], k=num_classes - 1) + np.diag([0.0] * num_classes, k=0)
...
def load_gmm_data():
    ...
    生成GMM数据集
    返回: (train_data, val_data, test_data)
    每个数据是一个列表，每个元素是一个numpy数组，形状为(n, 2)，其中第一列是时间，第二列是空间坐标
    """
    num_classes = 3
    alpha = 0.6
    m = np.array([0.1] * num_classes)
