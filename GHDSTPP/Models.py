...
import GHDSTPP.Constants as Constants
from GHDSTPP.Layers import EncoderLayer
from GHDSTPP.TemporalBackgroundEncoder import TemporalBackgroundEncoder
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.layer_stack = nn.ModuleList([
            EncoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout, normalize_before=False,loc_dim=loc_dim)
...
class Encoder_ST(nn.Module):
    ...
    def __init__(
        ...
        # 新增：时序自适应背景编码器（输出维度与d_model一致，便于融合）
        self.background_encoder = TemporalBackgroundEncoder(
            input_dim=1,
            hidden_dim=d_model,
            output_dim=d_model,
            num_layers=2,
    ...
    def forward(self, event_loc, event_time, non_pad_mask):
        ...
        background_feat = self.background_encoder(event_time, non_pad_mask)  # (batch, seq_len, d_model)
        ...
        enc_output = enc_output + background_feat 
        slf_attn_mask = slf_attn_mask[:,:,:,0]
        ...
        return enc_output, enc_output_temporal, enc_output_loc,background_feat
...
class Transformer_ST(nn.Module):
    ...
    def forward(self, event_loc, event_time):
        ...
        enc_output, enc_output_temporal, enc_output_loc, background_feat = self.encoder(event_loc, event_time, non_pad_mask)
        ...
        return enc_output_all, non_pad_mask, background_feat
